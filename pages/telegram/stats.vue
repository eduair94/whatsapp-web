<template>
  <div class="telegram-stats-page">
    <v-container>
      <!-- Header -->
      <div class="text-center mb-8">
        <h1>
          <v-icon class="mr-3" size="40">mdi-telegram</v-icon>
          {{ t("telegramStats.title") }}
        </h1>
        <p class="text-h6 text-medium-emphasis">{{ t("telegramStats.subtitle") }}</p>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="text-center my-16">
        <v-progress-circular :size="70" :width="7" color="primary" indeterminate></v-progress-circular>
        <p class="text-h6 mt-4">{{ t("telegramStats.loading") }}</p>
      </div>
        ...
        <v-row class="mb-8">
          <v-col cols="12" md="3">
            <v-card class="text-center pa-4" elevation="4">
              ...
              <p class="text-body-1 text-medium-emphasis">{{ t("telegramStats.totalNumbers") }}</p>
            </v-card>
          </v-col>
          <v-col cols="12" md="3">
            <v-card class="text-center pa-4" elevation="4">
              ...
              <p class="text-body-1 text-medium-emphasis">{{ t("telegramStats.last24Hours") }}</p>
            </v-card>
          </v-col>
          <v-col cols="12" md="3">
            <v-card class="text-center pa-4" elevation="4">
              ...
              <p class="text-body-1 text-medium-emphasis">{{ t("telegramStats.lastWeek") }}</p>
            </v-card>
          </v-col>
          <v-col cols="12" md="3">
            <v-card class="text-center pa-4" elevation="4">
              ...
              <p class="text-body-1 text-medium-emphasis">{{ t("telegramStats.syncDuration") }}</p>
            </v-card>
          </v-col>
        </v-row>
          ...
          <v-col cols="12">
            <v-card elevation="4" class="pa-6">
              ...
              <div class="map-container">
                <!-- Interactive Map -->
                <div v-if="!mapError" id="telegram-world-map" class="map-element">
          ...
          <v-col cols="12" md="4">
            <v-card elevation="4" class="pa-6">
              <v-card-title class="text-h5 mb-4">
                ...
                {{ t("telegramStats.photoStatistics") }}
              </v-card-title>
              <div class="chart-container">
                <Doughnut :data="photoChartData" :options="donutOptions" :height="250" />
              </div>
              <div class="mt-4">
                <div class="d-flex justify-space-between align-center mb-2">
                  <span>{{ t("telegramStats.withPhotos") }}</span>
                  <v-chip color="success" size="small">{{ formatNumber(statsData.photoStats.withPhotos) }}</v-chip>
                </div>
                <div class="d-flex justify-space-between align-center">
                  <span>{{ t("telegramStats.withoutPhotos") }}</span>
                  <v-chip color="error" size="small">{{ formatNumber(statsData.photoStats.withoutPhotos) }}</v-chip>
                </div>
              </div>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="4">
            <v-card elevation="4" class="pa-6">
              <v-card-title class="text-h5 mb-4">
                ...
                {{ t("telegramStats.accountTypes") }}
              </v-card-title>
              <div class="chart-container">
                <Doughnut :data="accountTypesChartData" :options="donutOptions" :height="250" />
              </div>
              <div class="mt-4">
                <div v-for="type in statsData.accountTypes" :key="type.type" class="d-flex justify-space-between align-center mb-2">
                  <span>{{ type.type }}</span>
                  <v-chip color="primary" size="small">{{ formatNumber(type.count) }}</v-chip>
                </div>
              </div>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="4">
            <v-card elevation="4" class="pa-6">
              <v-card-title class="text-h5 mb-4">
                ...
                {{ t("telegramStats.verificationStatus") }}
              </v-card-title>
              <div class="chart-container">
                <Doughnut :data="verificationChartData" :options="donutOptions" :height="250" />
              </div>
              <div class="mt-4">
                <div v-for="status in statsData.verificationStats" :key="status.type" class="d-flex justify-space-between align-center mb-2">
                  <span>{{ status.type }}</span>
                  <v-chip :color="status.type === 'Verified' ? 'success' : 'warning'" size="small">{{ formatNumber(status.count) }}</v-chip>
                </div>
              </div>
            </v-card>
          </v-col>
        ...
        <v-row class="mb-8">
          <v-col cols="12" md="6">
            <v-card elevation="4" class="pa-6">
              <v-card-title class="text-h5 mb-4">
                ...
                {{ t("telegramStats.premiumStatistics") }}
              </v-card-title>
              <div class="chart-container">
                <Bar :data="premiumChartData" :options="barChartOptions" :height="300" />
              </div>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="6">
            <v-card elevation="4" class="pa-6">
              <v-card-title class="text-h5 mb-4">
                <v-icon class="mr-2">mdi-table</v-icon>
                {{ t("telegramStats.summaryStatistics") }}
              </v-card-title>
              <v-table>
                <tbody>
                  <tr>
                    <td class="font-weight-bold">{{ t("telegramStats.totalUsers") }}</td>
                    <td class="text-right">{{ formatNumber(statsData.total) }}</td>
                  </tr>
                  <tr>
                    <td class="font-weight-bold">{{ t("telegramStats.photoPercentage") }}</td>
                    <td class="text-right">{{ statsData.photoStats.photoPercentage }}%</td>
                  </tr>
                  <tr>
                    <td class="font-weight-bold">{{ t("telegramStats.usersWithPhotos") }}</td>
                    <td class="text-right">{{ formatNumber(statsData.photoStats.withPhotos) }}</td>
                  </tr>
                  <tr>
                    <td class="font-weight-bold">{{ t("telegramStats.usersWithoutPhotos") }}</td>
                    <td class="text-right">{{ formatNumber(statsData.photoStats.withoutPhotos) }}</td>
                  </tr>
                  <tr>
                    <td class="font-weight-bold">{{ t("telegramStats.syncDuration") }}</td>
                    <td class="text-right">{{ Math.round(statsData.syncDurationMs / 1000) }}s</td>
                  </tr>
                </tbody>
              </v-table>
            </v-card>
          ...
          <v-col cols="12">
            <v-card elevation="2" variant="tonal" color="info">
              <v-card-text class="text-center">
                ...
                {{ t("telegramStats.lastSynchronized") }} {{ formatDate(statsData.lastUpdateDate) }}
                <v-chip class="ml-2" :color="statsData.fromCache ? 'warning' : 'success'" size="small">
                  {{ statsData.fromCache ? t("telegramStats.fromCache") : t("telegramStats.liveData") }}
                </v-chip>
                <span v-if="statsData.fromCache" class="ml-2 text-caption"> ({{ t("telegramStats.cacheAge") }} {{ Math.round(statsData.cacheAge / 1000) }}s) </span>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
...
// Head meta
useHead({
  title: computed(() => t("telegramStats.title")),
  meta: [{ name: "description", content: computed(() => t("telegramStats.subtitle")) }],
});
...
const fetchStats = async () => {
  try {
    loading.value = true;
    const response = await $fetch("/api/telegram/stats").catch(async (e) => {
      // Handle fetch errors
    ...
    if (response.success) {
      ...
      error.value = t("telegramStats.failedToLoad");
    }
  } catch (err) {
    error.value = t("telegramStats.errorFetching") + " " + err.message;
  } finally {
    loading.value = false;
  }
};
    ...
    tooltip: {
      ...
      callbacks: {
        label: function (context) {
          return `${t("telegramStats.numbers")}: ${formatNumber(context.parsed.y)}`;
        },
      },
    },
  },
...
const tableHeaders = [
  ...
  { title: computed(() => t("telegramStats.numbers")), key: "count", sortable: true },
...
// Utility function to get percentage color
...
const initWorldMap = async () => {
  ...
  try {
    ...
    const mapContainer = document.getElementById("telegram-world-map");
    console.log("Map container found:", mapContainer);
    if (!mapContainer) {
      console.log("Map container not found! Waiting and retrying...");
      await new Promise((resolve) => setTimeout(resolve, 500));
      const retryContainer = document.getElementById("telegram-world-map");
      if (!retryContainer) {
    ...
    // Create map
    leafletMap = L.map("telegram-world-map", {
      preferCanvas: true,
      zoomControl: true,
      ...
      const popupContent = `
        <div class="popup-container">
          ...
          <p class="popup-count popup-count-color" data-color="${color}">
            ${formatNumber(country.count)} ${t("telegramStats.numbers")}
          </p>
...
// Chart data for photo statistics
const photoChartData = computed(() => {
  if (!statsData.value?.photoStats) return { labels: [], datasets: [] };

  return {
    labels: [t("telegramStats.withPhotos"), t("telegramStats.withoutPhotos")],
    datasets: [
      {
        data: [statsData.value.photoStats.withPhotos, statsData.value.photoStats.withoutPhotos],
...
const accountTypesChartData = computed(() => {
  if (!statsData.value?.accountTypes) return { labels: [], datasets: [] };

  const labels = statsData.value.accountTypes.map((type) => type.type);
  const data = statsData.value.accountTypes.map((type) => type.count);
    ...
    datasets: [
      {
        ...
        borderColor: ["#1976D2", "#F57C00", "#7B1FA2"],
        borderWidth: 2,
      },
    ],
  };
...
const verificationChartData = computed(() => {
  if (!statsData.value?.verificationStats) return { labels: [], datasets: [] };

  const labels = statsData.value.verificationStats.map((status) => status.type);
  const data = statsData.value.verificationStats.map((status) => status.count);
...
// Chart data for premium statistics
const premiumChartData = computed(() => {
  if (!statsData.value?.premiumStats) return { labels: [], datasets: [] };

  const labels = statsData.value.premiumStats.map((status) => status.type);
  const data = statsData.value.premiumStats.map((status) => status.count);

  return {
    labels,
    datasets: [
      {
        label: t("telegramStats.users"),
        data,
