...
export function bypassServiceWorkerRedirect(url: string) {
  ...
  // Prevent infinite redirect loops if we're already on the refresh endpoint
  if (typeof window !== "undefined" && window.location.pathname.includes("/api/refresh")) {
    console.warn("Preventing redirect loop on api/refresh");
    return;
  }
    ...
    try {
      ...
      await Promise.all(
        cacheNames.map(async (cacheName) => {
          ...
          await Promise.all(requests.filter((request) => request.url.includes("/api/")).map((request) => cache.delete(request)));
        })
      );
    } catch (error) {
